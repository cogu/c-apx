...
static void test_program_end_on_empty_program(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, program, 0u));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_next_operation(&decoder, &operation_type));
...
static void test_parse_simple_pack_header_uint8(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_program_header(&encoder, APX_PACK_PROGRAM, 10u, 0u, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.header), adt_bytearray_length(&encoder.header)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_program_header(&decoder, &header));
...
static void test_parse_simple_unpack_header_uint16(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_program_header(&encoder, APX_UNPACK_PROGRAM, 1000u, 0u, true));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.header), adt_bytearray_length(&encoder.header)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_program_header(&decoder, &header));
...
static void test_parse_pack_header_uint32(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_program_header(&encoder, APX_PACK_PROGRAM, 100000u, 0u, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.header), adt_bytearray_length(&encoder.header)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_program_header(&decoder, &header));
...
static void test_parse_queued_header(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_program_header(&encoder, APX_PACK_PROGRAM, 2u, 4u, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.header), adt_bytearray_length(&encoder.header)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_program_header(&decoder, &header));
...
static void test_parse_invalid_header_version(CuTest* tc)
{
   ...
   apx_program_decoder_create(&decoder);
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, bad_header, sizeof(bad_header)));
   CuAssertIntEquals(tc, APX_INVALID_HEADER_ERROR, apx_program_decoder_parse_program_header(&decoder, &header));
   apx_program_decoder_destroy(&decoder);
}
...
static void test_parse_truncated_header(CuTest* tc)
{
   ...
   apx_program_decoder_create(&decoder);
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, truncated, sizeof(truncated)));
   CuAssertIntEquals(tc, APX_PARSE_ERROR, apx_program_decoder_parse_program_header(&decoder, &header));
   apx_program_decoder_destroy(&decoder);
}
...
static void test_decode_pack_scalar_types(CuTest* tc)
{
   ...
   for (size_t i = 0u; i < num_cases; ++i)
   {
      ...
      CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_PACK, test_cases[i].variant, false));
      CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.buffer), adt_bytearray_length(&encoder.buffer)));
      CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_next_operation(&decoder, &op_type));
      CuAssertIntEquals(tc, APX_OPERATION_TYPE_PACK, op_type);

      apx_program_decoder_get_pack_unpack_info(&decoder, &info);
...
static void test_decode_unpack_scalar_types(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_UNPACK, APX_VM_VARIANT_UINT32, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.buffer), adt_bytearray_length(&encoder.buffer)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_next_operation(&decoder, &op_type));
...
static void test_decode_fixed_array(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_PACK, APX_VM_VARIANT_UINT8, true));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_array_size(&encoder, 50u, false));
...
static void test_decode_dynamic_array(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_UNPACK, APX_VM_VARIANT_UINT16, true));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_array_size(&encoder, 2000u, true));
...
static void test_decode_range_checks_unsigned(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_limit_check_instruction(&encoder, APX_VM_VARIANT_LIMIT_CHECK_U32, 10, 50000, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.buffer), adt_bytearray_length(&encoder.buffer)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_next_operation(&decoder, &op_type));
   CuAssertIntEquals(tc, APX_OPERATION_TYPE_RANGE_CHECK_UINT32, op_type);

   apx_range_check_uint32_operation_info_t info = { 0u, 0u };
   apx_program_decoder_range_check_info_uint32(&decoder, &info);
   CuAssertUIntEquals(tc, 10u, info.lower_limit);
   CuAssertUIntEquals(tc, 50000u, info.upper_limit);
   CuAssertFalse(tc, apx_program_decoder_is_array_limit(&decoder));
...
static void test_decode_range_checks_signed(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_limit_check_instruction(&encoder, APX_VM_VARIANT_LIMIT_CHECK_S16, -1000, 1000, true));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.buffer), adt_bytearray_length(&encoder.buffer)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_next_operation(&decoder, &op_type));
   CuAssertIntEquals(tc, APX_OPERATION_TYPE_RANGE_CHECK_INT32, op_type);

   apx_range_check_int32_operation_info_t info = { 0, 0 };
   apx_program_decoder_range_check_info_int32(&decoder, &info);
   CuAssertIntEquals(tc, -1000, info.lower_limit);
   CuAssertIntEquals(tc, 1000, info.upper_limit);
   CuAssertTrue(tc, apx_program_decoder_is_array_limit(&decoder));
...
static void test_decode_record_select_and_end(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_DATA_CTRL, APX_VM_VARIANT_RECORD_SELECT, true));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_field_name(&encoder, "Field1"));

   // Second field: "Field2", is_first = false
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_DATA_CTRL, APX_VM_VARIANT_RECORD_SELECT, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_field_name(&encoder, "Field2"));

   // Record end
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_DATA_CTRL, APX_VM_VARIANT_RECORD_END, false));
   ...
   CuAssertStrEquals(tc, "Field1", apx_program_decoder_get_field_name(&decoder));
   CuAssertTrue(tc, apx_program_decoder_is_first_field(&decoder));

   // Parse second field
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_next_operation(&decoder, &op_type));
   CuAssertIntEquals(tc, APX_OPERATION_TYPE_RECORD_SELECT, op_type);
   CuAssertStrEquals(tc, "Field2", apx_program_decoder_get_field_name(&decoder));
   CuAssertFalse(tc, apx_program_decoder_is_first_field(&decoder));
...
static void test_decode_flow_ctrl_array_next(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_FLOW_CTRL, APX_VM_VARIANT_ARRAY_NEXT, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.buffer), adt_bytearray_length(&encoder.buffer)));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_parse_next_operation(&decoder, &op_type));
...
static void test_decode_position_save_and_recall(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_PACK, APX_VM_VARIANT_UINT8, false));
   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_encoder_encode_instruction(&encoder, APX_VM_OPCODE_PACK, APX_VM_VARIANT_UINT16, false));

   CuAssertIntEquals(tc, APX_NO_ERROR, apx_program_decoder_select_program(&decoder, adt_bytearray_data(&encoder.buffer), adt_bytearray_length(&encoder.buffer)));
   CuAssertFalse(tc, apx_program_decoder_has_saved_program_position(&decoder));
   ...
   apx_program_decoder_save_program_position(&decoder);
   CuAssertTrue(tc, apx_program_decoder_has_saved_program_position(&decoder));
   ...
   // Recall position
   apx_program_decoder_recall_program_position(&decoder);
