/*****************************************************************************
* \file      testsuite_file_map.c
* \author    Conny Gustafsson
* \date      2017-02-20
* \brief     Unit tests for file map
*
...
{
   apx_file_map_t map;
   apx_file_map_create(&map, false);
   rmf_file_info_t* info = rmf_file_info_make_fixed("TestNode1.out", 10u, RMF_INVALID_ADDRESS);
   apx_file_t const* file = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file);
   CuAssertUIntEquals(tc, 0u, apx_file_get_address(file));
   info = rmf_file_info_make_fixed("TestNode2.out", 10u, RMF_INVALID_ADDRESS);
   file = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file);
...
static void test_create_port_file_after_definition_file(CuTest* tc)
{
   apx_file_map_t map;
   apx_file_map_create(&map, false);
   rmf_file_info_t* info = rmf_file_info_make_fixed("TestNode1.apx", 100u, RMF_INVALID_ADDRESS);
   apx_file_t const* file1 = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file1);
   info = rmf_file_info_make_fixed("TestNode1.out", 10u, RMF_INVALID_ADDRESS);
   apx_file_t const* file2 = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file2);
   adt_list_elem_t* next = adt_list_iter_first(apx_file_map_get_list(&map));
...
static void test_add_too_many_files_of_same_type(CuTest* tc)
{
   ...
   rmf_file_info_t* info = rmf_file_info_make_fixed("TestNode1.apx", 10000u, RMF_INVALID_ADDRESS);
   apx_file_t const* file = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file);
   CuAssertUIntEquals(tc, APX_DEFINITION_ADDRESS_START, apx_file_get_address(file));
   ...
   for (i = 0; i < 64; i++)
   {
      ...
      info = rmf_file_info_make_fixed(name, file_size, RMF_INVALID_ADDRESS);
      file = apx_file_map_create_file(&map, info);
      CuAssertPtrNotNull(tc, file);
      rmf_file_info_delete(info);
   ...
   info = rmf_file_info_make_fixed("TestNode64.out", 1u, RMF_INVALID_ADDRESS);
   apx_file_t const* file2 = apx_file_map_create_file(&map, info);
...
static void test_find_file_by_address(CuTest* tc)
{
   ...
   info = rmf_file_info_make_fixed("TestNode3.out", 10u, RMF_INVALID_ADDRESS);
   apx_file_t const* file3 = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file3);
   CuAssertUIntEquals(tc, 2048u, apx_file_get_address(file3));
   info = rmf_file_info_make_fixed("TestNode1.apx", 100u, RMF_INVALID_ADDRESS);
   apx_file_t const* file4 = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file4);
   info = rmf_file_info_make_fixed("TestNode2.apx", 100u, RMF_INVALID_ADDRESS);
   apx_file_t const* file5 = apx_file_map_create_file(&map, info);
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file5);
   apx_file_t const* file = apx_file_map_find_by_address(&map, 2048);
   CuAssertConstPtrEquals(tc, file3, file);
   file = apx_file_map_find_by_address(&map, 2048); //This should used cached value
   CuAssertConstPtrEquals(tc, file3, file);
   file = apx_file_map_find_by_address(&map, 1024);
...
static void test_find_file_by_name(CuTest* tc)
{
   ...
   rmf_file_info_delete(info);
   CuAssertPtrNotNull(tc, file5);
   apx_file_t const* file = apx_file_map_find_by_name(&map, "TestNode1.apx");
   CuAssertConstPtrEquals(tc, file4, file);
   file = apx_file_map_find_by_name(&map, "TestNode1.out");
...
static void test_find_remote_file_by_local_address(CuTest* tc)
{
   ...
   file = apx_file_map_find_by_address(&map, 1024);
   CuAssertConstPtrEquals(tc, file2, file);
   file = apx_file_map_find_by_address(&map, 9u);
   CuAssertConstPtrEquals(tc, file1, file);
   apx_file_map_destroy(&map);
}
