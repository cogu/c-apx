...
static void test_empty_definition_data_is_initialized_on_new_instance(CuTest *tc)
{
   ...
   apx_node_manager_t* manager = apx_nodeManager_new(APX_SERVER_MODE);
   rmf_file_info_t* file_info = rmf_fileInfo_make_fixed("TestNode.apx", (uint32_t)definition_size, APX_DEFINITION_ADDRESS_START);
...
static void test_checksum_data_is_copied_from_file_info(CuTest* tc)
{
   ...
   rmf_file_info_t* file_info = rmf_fileInfo_make_fixed_with_digest("TestNode.apx", (uint32_t)definition_size, APX_DEFINITION_ADDRESS_START,
...
static void test_port_signature_uint8(CuTest* tc)
{
   ...
   apx_node_instance_t* node_instance = apx_nodeManager_get_last_attached(manager);
   apx_port_instance_t* port = apx_nodeInstance_get_provide_port(node_instance, 0);
