...
static void test_apx_fileManagerShared_create(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, 0, apx_fileManagerShared2_create(&shared));
...
static void test_apx_fileManagerShared_alloc(CuTest* tc)
{
   ...
   size_t size;
   int i;
   apx_fileManagerShared2_create(&shared);
   //allocate small objects
   for(i=1;i<SOA_SMALL_OBJECT_MAX_SIZE;i++)
   {
      char msg[20];
      size = i;
      ptr = apx_fileManagerShared2_alloc(&shared, size);
      sprintf(msg, "size=%d", i);
      CuAssertPtrNotNullMsg(tc, msg, ptr);
      apx_fileManagerShared2_free(&shared, ptr, size);
...
static void test_apx_fileManagerShared_findLocalFileByName(CuTest* tc)
{
   ...
   apx_fileManagerShared2_createLocalFile(&shared, &fileInfo1);
   apx_fileManagerShared2_createLocalFile(&shared, &fileInfo2);
   apx_fileManagerShared2_createLocalFile(&shared, &fileInfo3);
   CuAssertIntEquals(tc, 0, apx_fileManagerShared2_getNumRemoteFiles(&shared));
   CuAssertIntEquals(tc, 3, apx_fileManagerShared2_getNumLocalFiles(&shared));
   file = apx_fileManagerShared2_findLocalFileByName(&shared, "TestNode2.apx");
...
static void test_apx_fileManagerShared_findRemoteFileByName(CuTest* tc)
{
   ...
   apx_fileManagerShared2_createRemoteFile(&shared, &fileInfo1);
   apx_fileManagerShared2_createRemoteFile(&shared, &fileInfo2);
   apx_fileManagerShared2_createRemoteFile(&shared, &fileInfo3);
   CuAssertIntEquals(tc, 3, apx_fileManagerShared2_getNumRemoteFiles(&shared));
   CuAssertIntEquals(tc, 0, apx_fileManagerShared2_getNumLocalFiles(&shared));
   file = apx_fileManagerShared2_findRemoteFileByName(&shared, "TestNode2.apx");
   CuAssertPtrNotNull(tc, file);
...
static void test_apx_fileManagerShared_findLocalFileByAddress(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, 0, apx_fileManagerShared2_getNumRemoteFiles(&shared));
   CuAssertIntEquals(tc, 3, apx_fileManagerShared2_getNumLocalFiles(&shared));
   file = apx_fileManagerShared2_findFileByAddress(&shared, APX_ADDRESS_DEFINITION_START+0x100000);
   CuAssertPtrNotNull(tc, file);
...
static void test_apx_fileManagerShared_findRemoteFileByAddress(CuTest* tc)
{
   ...
   CuAssertIntEquals(tc, 3, apx_fileManagerShared2_getNumRemoteFiles(&shared));
   CuAssertIntEquals(tc, 0, apx_fileManagerShared2_getNumLocalFiles(&shared));
   file = apx_fileManagerShared2_findFileByAddress(&shared, (APX_ADDRESS_DEFINITION_START+0x100000) | RMF_REMOTE_ADDRESS_BIT);
   CuAssertPtrNotNull(tc, file);
   CuAssertStrEquals(tc, "TestNode2.apx", file->fileInfo.name);
   apx_fileManagerShared2_destroy(&shared);
   apx_fileInfo_destroy(&fileInfo1);
